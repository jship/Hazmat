...
#ifndef HAZMAT_MATH_VECTOR_TRANSFORM_H
#define HAZMAT_MATH_VECTOR_TRANSFORM_H
...
{
public:
    static Vector<2, T>& compute(const Matrix<4, T>& m,
                                 const Vector<2, T>& u,
                                       Vector<2, T>& v)
    {
        T x = u[X], y = u[Y];
...
{
public:
    static Vector<2, T>& compute(const Matrix<9, T>& m,
                                 const Vector<2, T>& u,
                                       Vector<2, T>& v)
    {
        T x = u[X], y = u[Y];
...
{
public:
    static Vector<2, T>& compute(const Matrix<16, T>& m,
                                 const Vector< 2, T>& u,
                                       Vector< 2, T>& v)
    {
        T x = u[X], y = u[Y];

        v[X] = m[12] + x * m[0] + y * m[2];
        v[Y] = m[13] + x * m[1] + y * m[3];
        return v;
    }
};
...
{
public:
    static Vector<3, T>& compute(const Matrix<9, T>& m,
                                 const Vector<3, T>& u,
                                       Vector<3, T>& v)
    {
        T x = u[X], y = u[Y], z = u[Z];
...
{
public:
    static Vector<3, T>& compute(const Matrix<16, T>& m,
                                 const Vector< 3, T>& u,
                                       Vector< 3, T>& v)
    {
        T x = u[X], y = u[Y], z = u[Z];

        v[X] = m[12] + x * m[0] + y * m[3] + z * m[6];
        v[Y] = m[13] + x * m[1] + y * m[4] + z * m[7];
        v[Z] = m[14] + x * m[2] + y * m[5] + z * m[8];
        return v;
    }
};
...
{
public:
    static Vector<4, T>& compute(const Matrix<16, T>& m,
                                 const Vector< 4, T>& u,
                                       Vector< 4, T>& v)
    {
